...
Codex Obsidian plugin 和 OpenAI Zotero Codex plugin 都以 submodule 形式放在本仓库：
...
`install_codex_plugins.sh` 会初始化 submodule，安装本仓库的 `paper-easy`、`codex-obsidian`，并在 OpenAI plugins submodule 可用时安装 `zotero@openai-curated`。
